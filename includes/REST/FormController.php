...
use DialogContactForm\Models\Entry;
...
class FormController extends Controller {
	...
	public function get_items( $request ) {
		...
		$entries_count = Entry::totalCount();

		/** @var ContactForm $form */
		foreach ( $forms as $form ) {
			$totalEntries        = isset( $entries_count[ $form->getId() ] ) ? $entries_count[ $form->getId() ] : 0;
			$response['items'][] = array(
				'id'      => $form->getId(),
				'title'   => $form->getTitle(),
				'date'    => self::formatDate( $form->getCreatedAt() ),
		...
		$response['meta']['pagination'] = $this->getPaginationMetadata( array(
			'totalCount' => isset( $counts['publish'] ) ? intval( $counts['publish'] ) : 0,
			'limit'      => $per_page,
			'offset'     => $offset,
		) );

		$response['meta']['sortedBy'] = $this->getSortingMetadata( $orderby, $order );
